...
public class ClientOptionInterpretor implements OptionInterpretor {
	...
	public Cmds interpret(String[] args) throws ParseException, NumberFormatException {
		...
		options.addOption("secure", false, "secure mode");
		...
		if (cmd.hasOption("port")) {
			// if the string does not contain a parsable integer, throw a
		...
		if (cmd.hasOption("servers")) {
			...
			for (String string : servers) {
				...
				if ((isStringDomain(DomainAndPort[0]) || isStringIPAddr(DomainAndPort[0]))
						&& (isStringPort(DomainAndPort[1])) && 2 == DomainAndPort.length) {
					if (isStringIPAddr(DomainAndPort[0])) {
						...
						// DomainAndPort[1] =
						// DomainAndPort[1].replaceFirst("0+(?!$)", "");
